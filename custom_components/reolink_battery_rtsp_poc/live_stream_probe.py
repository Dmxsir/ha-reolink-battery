"""Bounded, isolated Baichuan live-view probe for Reolink battery cameras.

The PoC intentionally imports only the production integration's inert UID/LAN
transport primitives.  It never imports beta download modules and never patches
the production recording path.  A probe is explicit, main-stream only, bounded
to 10 seconds by the button, and always closes the camera session afterward.
"""
...
# Safe dependency on the stable integration: transport.py has no beta monkey
# patches and provides the already-proven UID discovery + single-socket handoff.
from custom_components.reolink_battery.transport import (
    BAICHUAN_MAGIC,
    DISCOVERY_MAGIC,
...
def _prepare_standalone_channel_zero(host: Host) -> None:
    """Apply the same channel-0 metadata shim proven by the production path."""
...
def _scan_bcmedia(data: bytes, trace: LiveStreamTrace) -> bool:
    """Find BcMedia packet markers without retaining or exposing payload bytes."""
    found = False
...
def _extension_encrypt_len(extension: bytes) -> int | None:
    """Extract encryptLen from a small decrypted Extension without XML exposure."""
    start = extension.find(b"<encryptLen>")
    end = extension.find(b"</encryptLen>")
    if start < 0 or end <= start:
        return None
    start += len(b"<encryptLen>")
    try:
        return int(extension[start:end].strip())
    except ValueError:
        return None


def _encode_p2p_heartbeat(transaction_id: int, client_id: int, device_id: int) -> bytes:
    xml = (
        "<P2P><C2D_HB>"
...
class _LiveStreamProtocol(_IdempotentUdpClientProtocol):
    """Consume only the active cmd3/cmd4 stream; normal login traffic stays stock."""
    ...
    def parse_bc_data(self) -> None:
        ...
        if trace is not None and frame.cmd_id == LIVE_START_CMD_ID:
            ...
            if trace.first_cmd3_delay_ms is None and self._live_started_at is not None:
                trace.first_cmd3_delay_ms = round(
                    max(0.0, self._loop.time() - self._live_started_at) * 1000.0, 3
                )
        ...
        # Do not let reolink-aio reinterpret the special channel/stream/msgNum16
        # header as its generic 24-bit message-id layout.
        self._data = b""
...
class _LiveStreamConnection(BoundBaichuanUdpConnection):
    """Independent live-stream transport using the proven UID handoff socket."""
    ...
    def prepare_live_probe(
        self, decryptor: Callable[..., bytes | str], *, stream_kind: StreamKind
    ) -> LiveStreamTrace:
    ...
    def _send_heartbeat(self) -> bool:
        ...
        packet = _encode_p2p_heartbeat(
            transaction_id, protocol.client_id, protocol.host_id
        )
    ...
    async def send_live_stream_probe(
        ...
        *,
        ...
        try:
            await self.send_without_wait(start_wire, cmd_id=LIVE_START_CMD_ID, timeout=5)
            start_frame = await asyncio.wait_for(
                asyncio.shield(start_future), timeout=START_TIMEOUT_SECONDS
            )
            ...
            deadline = self._loop.time() + max(0.1, min(float(duration), MAX_PROBE_SECONDS))
            ...
            if self.connection_open:
                ...
                try:
                    await self.send_without_wait(stop_wire, cmd_id=LIVE_STOP_CMD_ID, timeout=2)
...
async def async_probe_live_stream(
    ...
    *,
    ...
    """Run one explicit bounded Preview session and return secret-safe telemetry."""
    lease = None
    host = None
    ...
    try:
        ...
        raise
    except (ReolinkError, OSError, TimeoutError, asyncio.TimeoutError, RuntimeError, ValueError) as err:
